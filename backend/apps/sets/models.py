...
class Node(MP_Node):
    ...
    display_type = models.CharField(max_length=20, db_index=True, choices=DisplayOptions.choices, blank=True, default=DisplayOptions.DEFAULT)
...
def group_media_queryset(qs, display_type=Node.DisplayOptions.DEFAULT):
    ...
    if display_type == dtypes.GROUPED_TITLE:
        ...
        qs = qs.order_by(Lower('creation_date')).annotate(grouper=Lower('creation_date'))
    return qs
